...
dx12::Cvars::Cvars()
{
	...
	vid_ref					= std::make_shared<Cvar>("vid_ref",							"dx12",													CVAR_ARCHIVE);
	...
	// DX Specific
	featureLevel			= std::make_shared<Cvar>("dx12_featureLevel",				"D3D_FEATURE_LEVEL_12_1",								CVAR_ARCHIVE);	// Leave this on 12_1, even on dx12
	...
	zFar2D					= std::make_shared<Cvar>("dx12_zFar2D",						99999,													CVAR_ARCHIVE);
	zFar3D					= std::make_shared<Cvar>("dx12_zFar3D",						4096,													CVAR_ARCHIVE);

#ifndef _DEBUG
	overlayScale			= std::make_shared<Cvar>("dx12_overlayScale",				1.0,													CVAR_ARCHIVE);
#else
	overlayScale			= std::make_shared<Cvar>("dx12_overlayScale",				0.9,													CVAR_ARCHIVE);
#endif
	overlayBrightness		= std::make_shared<Cvar>("dx12_overlayBrightness",			0.1125,													CVAR_ARCHIVE);
	overlayContrast			= std::make_shared<Cvar>("dx12_overlayContrast",			1.05,													CVAR_ARCHIVE);

	// Courtesy Kirk Barnes @ http://quake2xp.sourceforge.net/
	xpLitDownloadEnable		= std::make_shared<Cvar>("dx12_xpLitDownloadEnable",		false,													CVAR_ARCHIVE);
	xpLitDownloadPath		= std::make_shared<Cvar>("dx12_xpLitDownloadPath",			"https://sourceforge.net/p/quake2xp/code/HEAD/tree/",	CVAR_ARCHIVE);
	xpLitPathBaseQ2			= std::make_shared<Cvar>("dx12_xpLitPathBaseQ2",			"maps/",												CVAR_ARCHIVE);
	xpLitPathRogue			= std::make_shared<Cvar>("dx12_xpLitPathRogue",				"mapsr/",												CVAR_ARCHIVE);
...
dx12::Cvars::Cvar::Cvar(std::string name, float defaultValue, unsigned int flags)
{
	LOG_FUNC();
...
dx12::Cvars::Cvar::Cvar(std::string name, double defaultValue, unsigned int flags)
{
	LOG_FUNC();
...
dx12::Cvars::Cvar::Cvar(std::string name, int defaultValue, unsigned int flags)
{
	LOG_FUNC();
...
std::string dx12::Cvars::Cvar::String()
{
	// Wait for exclusive access
	std::lock_guard<std::mutex> lock(m_ptrAccessMutex);
...
std::string dx12::Cvars::Cvar::LatchedString()
{
	// Wait for exclusive access
	std::lock_guard<std::mutex> lock(m_ptrAccessMutex);
...
std::string dx12::Cvars::Cvar::Name()
{
	// Wait for exclusive access
	std::lock_guard<std::mutex> lock(m_ptrAccessMutex);
...
inline std::string dx12::Cvars::Cvar::CvarFlagsToString(unsigned int flags)
{
...
bool dx12::Cvars::Cvar::InfoValidate(std::string value)
{
	LOG_FUNC();
...
void dx12::Cvars::Cvar::Set(std::string value)
{
	LOG_FUNC();
