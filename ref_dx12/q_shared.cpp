...
void Com_sprintf(char *dest, int size, char *fmt, ...)
{
	...
	va_end(argptr);
	if (len >= size)
		Com_Printf(const_cast<char*>("Com_sprintf: overflow of %i in %i\n"), len, size);
	strncpy(dest, bigbuffer, static_cast<size_t>(size) - 1);
}
...
void Info_SetValueForKey(char *s, char *key, char *value)
{
	...
	{
		Com_Printf(const_cast<char*>("Can't use keys or values with a \\\n"));
		return;
	}

	if (strstr(key, ";"))
	{
		Com_Printf(const_cast<char*>("Can't use keys or values with a semicolon\n"));
		return;
	}
	...
	{
		Com_Printf(const_cast<char*>("Keys and values must be < 64 characters.\n"));
		return;
	}
	...
	Com_sprintf(newi, sizeof(newi), const_cast<char*>("\\%s\\%s"), key, value);

	if (strlen(newi) + strlen(s) > maxsize)
	{
		Com_Printf(const_cast<char*>("Info string length exceeded\n"));
		return;
	}
