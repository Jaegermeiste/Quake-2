...
void CL_WriteDemoMessage (void)
{
	...
	len = (long)net_message.cursize-8;
...
void CL_Record_f (void)
{
	...
	for (i=0 ; i<MAX_CONFIGSTRINGS ; i++)
	{
		if (cl.configstrings[i][0])
		{
			...
			{	// write it out
				len = LittleLong ((long)buf.cursize);
				FS_Write (&len, 4, cls.demofile);
...
void CL_WriteConfiguration (void)
{
	...
	if (f && f->fileHandle)
	{
		fprintf(f->fileHandle, "// generated by quake, do not modify\n");
		Key_WriteBindings(f);
	}
	FS_FCloseFile (f);
