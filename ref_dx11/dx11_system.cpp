...
void dx11::System::GetHardwareAdapter(IDXGIFactory2* pFactory, IDXGIAdapter1** ppAdapter)
{
	...
	*ppAdapter = nullptr;
	...
	for (UINT adapterIndex = 0; DXGI_ERROR_NOT_FOUND != pFactory->EnumAdapters1(adapterIndex, &adapter); ++adapterIndex)
	{
		...
		// Check to see if the adapter supports Direct3D 11, but don't create the
		// actual device yet.
		if (SUCCEEDED(D3D11CreateDevice(adapter.Get(), FeatureLevelForString(ref->cvars->featureLevel->String()), _uuidof(ID3D11Device), nullptr)))
		{
			break;
		}
	}
