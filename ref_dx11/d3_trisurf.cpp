...
srfTriangles_t *R_AllocStaticTriSurf() {
	srfTriangles_t *tris = new srfTriangles_t(); // (srfTriangles_t *)Mem_ClearedAlloc(sizeof(srfTriangles_t), TAG_SRFTRIS);
	return tris;
}
...
void R_AllocStaticTriSurfVerts(srfTriangles_t *tri, size_t numVerts) {
	...
	tri->verts = new dxVertex[numVerts](); //(idDrawVert *)Mem_Alloc16(numVerts * sizeof(idDrawVert), TAG_TRI_VERTS);
}
...
void R_AllocStaticTriSurfIndexes(srfTriangles_t *tri, size_t numIndexes) {
	...
	tri->indexes = new triIndex_t[numIndexes](); //(triIndex_t *)Mem_Alloc16(numIndexes * sizeof(triIndex_t), TAG_TRI_INDEXES);
}
...
=================
*/
void R_AllocStaticTriSurfDominantTris(srfTriangles_t *tri, unsigned int numVerts) {
	assert(tri->dominantTris == nullptr);
	tri->dominantTris = new dominantTri_t[numVerts](); //(dominantTri_t *)Mem_Alloc16(numVerts * sizeof(dominantTri_t), TAG_TRI_DOMINANT_TRIS);
}
...
=================
*/
void R_AllocStaticTriSurfMirroredVerts(srfTriangles_t *tri, unsigned int numMirroredVerts) {
	assert(tri->mirroredVerts == nullptr);
	tri->mirroredVerts = new int[numMirroredVerts](); // (int *)Mem_Alloc16(numMirroredVerts * sizeof(*tri->mirroredVerts), TAG_TRI_MIR_VERT);
}
...
=================
*/
void R_AllocStaticTriSurfDupVerts(srfTriangles_t *tri, unsigned int numDupVerts) {
	assert(tri->dupVerts == nullptr);
	tri->dupVerts = new int[numDupVerts * 2]();	//(int *)Mem_Alloc16(numDupVerts * 2 * sizeof(*tri->dupVerts), TAG_TRI_DUP_VERT);
}
...
=================
*/
void R_ResizeStaticTriSurfVerts(srfTriangles_t *tri, unsigned int numVerts) {
	dxVertex * newVerts = new dxVertex[numVerts](); //(idDrawVert *)Mem_Alloc16(numVerts * sizeof(idDrawVert), TAG_TRI_VERTS);
	const int copy = std::min(numVerts, tri->numVerts);
	memcpy(newVerts, tri->verts, copy * sizeof(dxVertex));
	delete[] tri->verts;
	tri->verts = nullptr;
	tri->verts = newVerts;
	newVerts = nullptr;
...
=================
*/
void R_ResizeStaticTriSurfIndexes(srfTriangles_t *tri, unsigned int numIndexes) {
	triIndex_t * newIndexes = new triIndex_t[numIndexes]();	//(triIndex_t *)Mem_Alloc16(numIndexes * sizeof(triIndex_t), TAG_TRI_INDEXES);
...
void R_RangeCheckIndexes(const srfTriangles_t *tri) {
	...
	if (tri->numIndexes < 0) {
		LOG(error) << "R_RangeCheckIndexes: numIndexes < 0";
	}
	if (tri->numVerts < 0) {
		LOG(error) << "R_RangeCheckIndexes: numVerts < 0";
	}

	// must specify an integral number of triangles
	if (tri->numIndexes % 3 != 0) {
		LOG(error) << "R_RangeCheckIndexes: numIndexes %% 3";
	}

	for (i = 0; i < tri->numIndexes; i++) {
		if (tri->indexes[i] >= tri->numVerts) {
			LOG(error) << "R_RangeCheckIndexes: index out of range";
		}
	}
...
void R_BoundTriSurf(srfTriangles_t *tri) {
	...
	BoundingOrientedBox::CreateFromPoints(tri->bounds, tri->numVerts, &tri->verts->xyz, sizeof(dxVertex));
}
...
static bool R_FaceNegativePolarity(const srfTriangles_t *tri, int firstIndex) {
	const dxVertex * a = tri->verts + tri->indexes[firstIndex + 0];
	const dxVertex * b = tri->verts + tri->indexes[firstIndex + 1];
	const dxVertex * c = tri->verts + tri->indexes[firstIndex + 2];

	const XMFLOAT2 aST = a->textureCoordinate0;
	const XMFLOAT2 bST = b->textureCoordinate0;
	const XMFLOAT2 cST = c->textureCoordinate0;
...
static void	R_DuplicateMirroredVertexes(srfTriangles_t *tri) {
	...
	int				totalVerts;
	int				numMirror;
	tangentVert_t	tanVertZero;
	memset(&tanVertZero, 0, sizeof(tangentVert_t));
	std::vector<tangentVert_t> tverts(tri->numVerts, tanVertZero);
...
void R_DeriveNormalsAndTangents(srfTriangles_t *tri) {
	XMVECTOR ClearVector = XMVectorZero();
	std::vector<XMVECTOR> vertexNormals (tri->numVerts, ClearVector);
	std::vector<XMVECTOR> vertexTangents (tri->numVerts, ClearVector);
	std::vector<XMVECTOR> vertexBitangents (tri->numVerts, ClearVector);

	for (unsigned int i = 0; i < tri->numIndexes; i += 3) {
		const int i0 = tri->indexes[i + 0];
		const int i1 = tri->indexes[i + 1];
		const int i2 = tri->indexes[i + 2];

		// Vertices
		const XMVECTOR v0 = XMVectorSet((tri->verts + i0)->xyz.x, (tri->verts + i0)->xyz.y, (tri->verts + i0)->xyz.z, 0.0);
		const XMVECTOR v1 = XMVectorSet((tri->verts + i1)->xyz.x, (tri->verts + i1)->xyz.y, (tri->verts + i1)->xyz.z, 0.0);
		const XMVECTOR v2 = XMVectorSet((tri->verts + i2)->xyz.x, (tri->verts + i2)->xyz.y, (tri->verts + i2)->xyz.z, 0.0);
		...
		// Edge Components
		const float x1 = XMVectorGetX(e0);
		const float x2 = XMVectorGetX(e1);
		const float y1 = XMVectorGetY(e0);
		const float y2 = XMVectorGetY(e1);
		const float z1 = XMVectorGetZ(e0);
		const float z2 = XMVectorGetZ(e1);

		// TexCoords
		const XMVECTOR w0 = XMVectorSet((tri->verts + i0)->textureCoordinate0.x, (tri->verts + i0)->textureCoordinate0.y, 0.0, 0.0);
		const XMVECTOR w1 = XMVectorSet((tri->verts + i1)->textureCoordinate0.x, (tri->verts + i1)->textureCoordinate0.y, 0.0, 0.0);
		const XMVECTOR w2 = XMVectorSet((tri->verts + i2)->textureCoordinate0.x, (tri->verts + i2)->textureCoordinate0.y, 0.0, 0.0);

		// ST
		const XMVECTOR st0 = w1 - w0;
		const XMVECTOR st1 = w2 - w0;

		const float s1 = XMVectorGetX(st0);
		const float t1 = XMVectorGetY(st0);
		const float s2 = XMVectorGetX(st1);
		const float t2 = XMVectorGetY(st1);

		// Normal from Edges
		const XMVECTOR normal = XMVector3Normalize(XMVector3Cross(e0, e1));

		// Tangent and Bitangent
		const float r = 1.0F / XMVectorGetX(XMVector2Cross(st0, st1));
		const XMVECTOR sTan = XMVectorSet((t2 * x1 - t1 * x2) * r, (t2 * y1 - t1 * y2) * r, (t2 * z1 - t1 * z2) * r, 0.0);
		const XMVECTOR tTan = XMVectorSet((s1 * x2 - s2 * x1) * r, (s1 * y2 - s2 * y1) * r, (s1 * z2 - s2 * z1) * r, 0.0);

		// Calculate handedness
		const float Tw = (XMVectorGetX(XMVector3Dot(XMVector3Cross(normal, sTan), tTan)) < 0.0F) ? -1.0F : 1.0F;
		XMVectorSetW(sTan, Tw);

		vertexNormals[i0] += normal;
		vertexTangents[i0] += sTan;
		vertexBitangents[i0] += tTan;

		vertexNormals[i1] += normal;
		vertexTangents[i1] += sTan;
		vertexBitangents[i1] += tTan;

		vertexNormals[i2] += normal;
		vertexTangents[i2] += sTan;
		vertexBitangents[i2] += tTan;
	}

	// add the normal of a duplicated vertex to the normal of the first vertex with the same XYZ
	for (unsigned int i = 0; i < tri->numDupVerts; i++) {
	...
	for (unsigned int i = 0; i < tri->numVerts; i++) {
		vertexNormals[i] = XMVector3Normalize(vertexNormals[i]);

		// Gram-Schmidt orthogonalize
		vertexTangents[i] = XMVector3Normalize(vertexTangents[i] - vertexNormals[i] * XMVector3Dot(vertexNormals[i], vertexTangents[i]));

		// Calculate handedness
		const float Tw = (XMVectorGetX(XMVector3Dot(XMVector3Cross(vertexNormals[i], vertexTangents[i]), vertexBitangents[i])) < 0.0F) ? -1.0F : 1.0F;
		XMVectorSetW(vertexTangents[i], Tw);

		// Recalculate bitangent
		vertexBitangents[i] = XMVector3Normalize(XMVector3Cross(vertexNormals[i], vertexTangents[i]) * Tw);
	}

	// compress the normals and tangents
	for (unsigned int i = 0; i < tri->numVerts; i++) {
		XMStoreFloat3(&(tri->verts[i].normal), vertexNormals[i]);
		XMStoreFloat4(&(tri->verts[i].tangent), vertexTangents[i]);
		XMStoreFloat3(&(tri->verts[i].bitangent), vertexBitangents[i]);
	}
}
...
void R_BuildDominantTris(srfTriangles_t *tri) {
	...
	indexSort_t *ind = new indexSort_t[numIndexes];
	if (ind == nullptr) {
		dx11::ref->client->Sys_Error(ERR_FATAL, "Couldn't allocate index sort array");
		return;
	}
...
void R_RemoveDuplicatedTriangles(srfTriangles_t *tri) {
	...
	if (c_removed) {
		dx11::ref->client->Con_Printf(PRINT_DEVELOPER, "removed " + std::to_string(c_removed) + " duplicated triangles\n");
	}
}
...
void R_RemoveDegenerateTriangles(srfTriangles_t *tri) {
	...
	if (c_removed) {
		dx11::ref->client->Con_Printf(PRINT_DEVELOPER, "removed " + std::to_string(c_removed) + " degenerate triangles\n");
	}
}
...
void R_TestDegenerateTextureSpace(srfTriangles_t *tri) {
	unsigned int		c_degenerate = 0;

	// check for triangles with a degenerate texture space
	for (unsigned int i = 0; i < tri->numIndexes; i += 3) {
		const XMVECTOR a = XMVectorSet(tri->verts[tri->indexes[i + 0]].textureCoordinate0.x, tri->verts[tri->indexes[i + 0]].textureCoordinate0.y, 0.0, 0.0);
		const XMVECTOR b = XMVectorSet(tri->verts[tri->indexes[i + 1]].textureCoordinate0.x, tri->verts[tri->indexes[i + 1]].textureCoordinate0.y, 0.0, 0.0);
		const XMVECTOR c = XMVectorSet(tri->verts[tri->indexes[i + 2]].textureCoordinate0.x, tri->verts[tri->indexes[i + 2]].textureCoordinate0.y, 0.0, 0.0);

		if (XMVector2Equal(a, b) || XMVector2Equal(b, c) || XMVector2Equal (c, a)) {
			c_degenerate++;
		}
	}
...
void R_RemoveUnusedVerts(srfTriangles_t *tri) {
	...
	mark = new int[tri->numVerts](); // R_ClearedStaticAlloc(tri->numVerts * sizeof(*mark));

	for (i = 0; i < tri->numIndexes; i++) {
		index = tri->indexes[i];
		if (index < 0 || index >= tri->numVerts) {
			dx11::ref->client->Sys_Error(ERR_DROP, "R_RemoveUnusedVerts: bad index");
		}
		mark[index] = 1;
...
===================
*/
deformInfo_t *R_BuildDeformInfo(int numVerts, const dxVertex *verts, int numIndexes, const int *indexes, bool useUnsmoothedTangents)
{
	srfTriangles_t	tri;
	memset(&tri, 0, sizeof(srfTriangles_t));
	...
	deformInfo_t * deform = new deformInfo_t();
	...
	deform->staticAmbientCache = vertexCache.AllocStaticVertex(deform->verts, ALIGN(deform->numOutputVerts * sizeof(dxVertex), VERTEX_CACHE_ALIGN));
...
void R_FreeDeformInfo(deformInfo_t *deformInfo) {
	...
	if (deformInfo->mirroredVerts != nullptr) {
		delete[] deformInfo->mirroredVerts;
	}
	if (deformInfo->dupVerts != nullptr) {
		delete[] deformInfo->dupVerts;
	}
